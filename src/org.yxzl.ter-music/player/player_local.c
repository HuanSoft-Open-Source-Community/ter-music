...
{
    return g_app_config.remote_connection_count;
}

int player_remote_server_get(int index, RemoteConnectionConfig *out)
{
    if (index < 0 || index >= g_app_config.remote_connection_count || !out) {
        return -1;
    }
    *out = g_app_config.remote_connections[index];
    return 0;
}
...
int player_remote_save_server(int index, const char *json)
{
    (void)index;
    (void)json;
    /* 本地后端下服务器条目由设置页直接改 g_app_config + save_config()，
     * 与远程后端（Config/Remote 接口）语义一致，故这里不再重复实现。 */
    g_local.config_revision++;
    return 0;
}
...
int player_remote_list(int index, const char *subpath)
{
    ...
    if (player_remote_server_get(index, &connection) != 0) {
        return -1;
    }
    return remote_list_directory(&connection, subpath, NULL, NULL) == 0 ? 0 : -1;
}
...
int player_remote_connect(int index, const char *subpath)
{
    ...
    int loaded = load_remote_playlist(&connection,
                                      (subpath && subpath[0]) ? subpath : connection.base_path);
    refresh_snapshot();
    g_local.playlist_revision++;
    return loaded > 0 ? 0 : -1;
}
